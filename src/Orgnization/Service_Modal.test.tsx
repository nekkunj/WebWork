...
import { updatingServiceParameterDetailsAPI } from '../state/new actions/organizationAction';
...
jest.mock("../state/new actions/organizationAction", () => ({
  updatingServiceParameterDetailsAPI: jest.fn(() => Promise.resolve()),
}));
...
describe('ServiceModal', () => {
    ...
    it('renders correctly', () => {
      ...
      render(<ServiceModal isOpen={true} handleClose={handleClose} document={documentMock} orgId={"123"}/>);
    ...
    it('checks for the parameters', () => {
        const handleClose = jest.fn();
        const {queryByText}=render(<ServiceModal isOpen={true} handleClose={handleClose} document={documentMock} orgId={"123"} />);
    ...
    it('submits the form with updated values', async () => {
        ...
        await waitFor(() => {
          expect(updatingServiceParameterDetailsAPI).toHaveBeenCalledWith(expect.anything(),expect.anything(),expect.anything());
        });
      ...
      it('No Parameters', () => {
        const handleClose = jest.fn();
        render(<ServiceModal isOpen={true} handleClose={handleClose} document={anotherdocumentMock} orgId={"123"} />);
