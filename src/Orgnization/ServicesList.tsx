...
import { fetchListOfUnassignedServices } from "../state/new actions/organizationAction";
import { deleteAssignedService } from "../state/new actions/organizationAction";
...
interface IServiceList{
  // cardData:{ serviceName: string; assignedOn: string; parameters: string; }[]
...
function ServicesList({cardData,orgId,assignedServices,prevStep,nextStep,prevStepExists,nextStepExist,fetchData,...props}:IServiceList){
  ...
  const handleDelete = async(index:number) => {
    ...
    await deleteAssignedService(Assignedservices[index],orgId,getToken(),handleLogoutRedirect)
  ...
  useEffect(() => {
    const fetchingUnassignedServices=async ()=>{
      await fetchListOfUnassignedServices(cardData,getToken(),handleLogoutRedirect)
    }
  ...
  useEffect(()=>{
    const fetchingUnassignedServices=async (newData:any)=>{
      await fetchListOfUnassignedServices(newData,getToken(),handleLogoutRedirect)
    }
  ...
  useEffect(()=>{
    ...
    height="200" src="https://www.youtube.com/"> </iframe> */}
            ...
            {Assignedservices.map((card,index)=>(
                ...
                <Card 
                  ...
                  actions={[
                            ...
                            <EditOutlined data-testid="edit-service" key="setting" onClick={()=>{handleServiceModelOpen(index)}} style={{color:'blue'}} />,
                            <DeleteOutlined data-testid="delete-service" style={{color:'red',fontSize:17}} onClick={() => handleDelete(index)}/>
                          ]}  
                >
        ...
        <Row justify={'end'}>
          ...
          { ServiceListOpen && <ServiceModal orgId={orgId} isOpen={ServiceListOpen} document={Assignedservices[serviceindex]} handleClose={handleServiceModelClose} />}
          {addServiceOpen && <AddServiceModal assignService={assignService} orgId={orgId} assignedServices={assignedServices} isOpen={addServiceOpen}  handleClose={close_AddServiceModel} />}
          <Modal 
