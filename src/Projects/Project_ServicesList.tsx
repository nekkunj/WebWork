...
import {  fetchUnassignedOrganizationServicesList } from "../state/new actions/projectAction";
...
function ServicesList({cardData,projectId,orgId,prevStep,nextStep,prevStepExists,nextStepExist,fetchData,...props}:IServiceList){
  ...
  const open_AddServiceModel=async()=>{
            ...
            {Assignedservices.map((card,index)=>(
                ...
                <Card 
                  ...
                  actions={[
                            ...
                            <EditOutlined data-testid="edit-service" key="setting" style={{color:'blue'}} onClick={()=>{handleServiceModelOpen(index)}} />,
                            <DeleteOutlined data-testid="delete-project-service" style={{color:'red',fontSize:17}} onClick={() => handleDelete(index)}/>
                          ]}  
                >
