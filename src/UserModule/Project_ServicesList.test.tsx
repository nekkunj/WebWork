...
import { deleteAssignedService } from "../state/new actions/projectAction";
...
describe('General User ServicesList Project Component', () => {
    it('renders correctly with initial data', () => {
        const {getByText,queryByRole}= render(
            <Provider store={store}>
                <ServicesList userRole="OrganizationAdmin" 
    ...
    it('renders correctly with reader/writer', () => {
        const {getByText,queryByRole}= render(
            <Provider store={store}>
                <ServicesList userRole="Reader" 
                    cardData={mockCardData} 
        ...
        expect(screen.queryByText('Assign a new service')).not.toBeInTheDocument(); //Not access
    ...
    it('handles no services', () => {
        const {getByText,queryByRole}= render(
            <Provider store={store}>
                <ServicesList userRole="ProjectAdmin" 
                    cardData={[]} 
    ...
    it('handles service deletion', async () => {
        ...
        userEvent.click(screen.getByTestId('delete-project-service')); // Ensure you have data-testid attributes in your component
        await waitFor(() => {
            expect(deleteAssignedService).toHaveBeenCalledWith(mockCardData[0],expect.anything(),expect.anything(),expect.anything());
        });
    });

    it('handles service deletion no access', async () => {
        render(
            <Provider store={store}>
                <ServicesList userRole="ProjectAdmin" 
                    cardData={mockCardData} 
        ...
        // Assert that the delete button with data-testid "delete-project-service" is not present
        // expect(screen.queryByTestId('delete-project-service')).toBeNull(); //It will be present but users can't click it
        await waitFor(() => {
