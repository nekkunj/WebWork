...
const cardData: { userName: string; assignedOn: string; role: string; }[] = [];

for (let i = 1; i <= 10; i++) {
  const obj = {
    userName: `User_${i}`,
    assignedOn: `24 Jan 2023`,
    role: `Writer`
  };
  cardData.push(obj);
}
...
function Project_Users_List_Card({cardData,organizationId,orgName,projectName,projectId,prevStep,nextStep,prevStepExists,nextStepExist,...props}:IUsers_List_Card){
    ...
    useEffect(()=>{
      if(cardData){
        setAssignedusers(cardData.filter((obj:any) => obj.projectId === projectId))
      }
      if (st) {
        const { instance }=st
    ...
    return(
        <div >
        <Row justify={"start"} align={'top'} style={{ display:'flex',alignItems:'flex-start',alignContent:'flex-start', height: '72vh', overflow: 'auto',paddingRight:'1vw'}}>
            ...
            {props.projectUpdateInProgress==false && AssignedUsers.map((card,index)=>(
                <Col xs={24} lg={22}>
                <Card key={index} style={{marginBottom:'5px',padding:'0px'}} >
